...
const
  IDENTIFYBUFLEN = 50; // default buffer length

  _IDTAGS = $CD450000;

  //Expansion() tags
  IDTAG_ConfigDev   = _IDTAGS + $00; // (PConfigDev) ConfigDev structure to be evaluated
  IDTAG_ManufID     = _IDTAGS + $01; // (LongWord) manufacturer ID if no ConfigDev is available
  IDTAG_ProdID      = _IDTAGS + $02; // (Byte) product ID if no ConfigDev is available
  IDTAG_StrLength   = _IDTAGS + $03; // (Word) of maximum buffer length, including termination. Default is 50.
  IDTAG_ManufStr    = _IDTAGS + $04; // (STRPTR) of manufacturer name buffer or nil
  IDTAG_ProdStr     = _IDTAGS + $05; // (STRPTR) of product name buffer, or nil
  IDTAG_ClassStr    = _IDTAGS + $06; // (STRPTR) of product class buffer, or nil
  ...
  IDTAG_Expansion   = _IDTAGS + $0C; // (PPConfigDev) for a complete expansion check. Init the variable with nil.
  IDTAG_ClassID     = _IDTAGS + $0E; // (PLongWord) class ID of the provided expansion board (see below) [V8]
  IDTAG_Localize    = _IDTAGS + $0F; // (BOOL) return localized strings (defaults to True) [V8]
  IDTAG_NULL4NA     = _IDTAGS + $10; // (BOOL) return nil for not available else return a string (defaults to False) [V8]
  IDTAG_UnknownFlag = _IDTAGS + $11; // (PByte) A flag that will be set to True if the current expansion was not
                                     // found in the database. The content will not be changed if the expansion
                                     // was found. [V39]
  IDTAG_Delegate    = _IDTAGS + $12; // (BOOL) True if unknown boards shall be delegated to other databases.
                                     //         False to skip checking other databases
  ...
  //  Be prepared to get other numbers as well. In this case you should assume IDCID_MISC.

  //  Error codes - Positive error codes are DOS errors!
...
function IdFunction(LibName: STRPTR location 'a0'; Offset: LongInt location 'd0'; Tags: PTagItem location 'a1'): LongInt; syscall IdentifyBase 48;
function IdHardwareNum(_Type: LongWord location 'd0'; Tags: PTagItem location 'a0'): LongWord syscall IdentifyBase 54;
...
function IdHardwareTags(_Type: LongWord; const TagList: array of PtrUInt): STRPTR;
function IdAlertTags(ID: LongWord; const TagList: array of PtrUInt): LongInt;
function IdFunctionTags(LibName: STRPTR; Offset: LongInt; const TagList: array of PtrUInt): LongInt;
function IdHardwareNumTags(_Type: LongWord; const TagList: array of PtrUInt): LongWord;
function IdFormatStringTags(_String: STRPTR; Buffer: STRPTR; Length: LongWord; const TagList: array of PtrUInt): LongWord;
function IdEstimateFormatSizeTags(_String: STRPTR; const TagList: array of PtrUInt): LongWord;
...
function IdExpansionTags(const TagList: array of PtrUInt): LongInt;
begin
  Result := IdExpansion(@TagList);
end;

function IdHardwareTags(_Type: LongWord; const TagList: array of PtrUInt): STRPTR;
begin
  Result := IdHardware(_Type, @TagList);
end;

function IdAlertTags(ID: LongWord; const TagList: array of PtrUInt): LongInt;
begin
  Result := IdAlert(ID, @TagList);
end;

function IdFunctionTags(LibName: STRPTR; Offset: LongInt; const TagList: array of PtrUInt): LongInt;
begin
  Result := IdFunction(LibName, Offset, @TagList);
end;

function IdHardwareNumTags(_Type: LongWord; const TagList: array of PtrUInt): LongWord;
begin
  Result := IdHardwareNum(_Type, @TagList);
end;

function IdFormatStringTags(_String: STRPTR; Buffer: STRPTR; Length: LongWord; const TagList: array of PtrUInt): LongWord;
begin
  Result := IdFormatString(_String, Buffer, Length, @TagList);
end;

function IdEstimateFormatSizeTags(_String: STRPTR; const TagList: array of PtrUInt): LongWord;
begin
  Result := IdEstimateFormatSize(_String, @TagList);
end;

initialization
  IdentifyBase := OpenLibrary(IdentifyLibName, IdentifyLibMinVersion);
finalization
